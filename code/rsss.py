...
for name, url in feeds.items():
    ...
    # 使用 BeautifulSoup 解析 XML 內容，並使用 lxml 解析器
    ...
    if feed_link:
        ...
        feed_domain = urllib.parse.urlparse(url).hostname
        encoded_feed_domain = urllib.parse.quote_plus(feed_domain)
    ...
    image_tag.find('url', {'url': True}).string = CData(f"https://images.weserv.nl/?n=-1&url={urllib.parse.quote_plus('https://external-content.duckduckgo.com/ip3/' + encoded_feed_domain + '.ico')}")
    ...
    for item in root.find_all('item'):
        ...
        if description is not None:
            ...
            # 使用 BeautifulSoup 找到所有的 <img> 標籤，並使用 lxml 解析器
            img_tags = [img['src'] for img in BeautifulSoup(description_text, 'lxml').find_all('img')]
